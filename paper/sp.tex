...
using the \gaia\ Archive official match (\citealt{xmatch}) to the \zmass\ IDs.
...
\begin{figure}
\includegraphics[width=\textwidth]{residuals_inclduing_parent_sample.pdf}
\caption{The predictive accuracy of the model, in the two-fold cross-validation.
  ...
  The points are colored by spectroscopic (\apogee) signal-to-noise ratio.
  The left panel shows the full Parent Sample,
  the middle panel shows the Training Set (both A and B combined),
...
to span some range in metallicity, age, and extinction, and therefore test
...
In detail, the comparison with the more recent sample (\citealt{que})
is better:
...
(at least approximately), they could have delivered better---or at least similar---results.
...
the simple fact that the problem is \emph{close} to linear.
A more complex model might also have edge issues, because flexible models don't extrapolate
outside the convex hull of their training data.
...
the linear model contains essentially only first derivatives in the data space.
...
And it means that we did not obtain as much insight from the spectroscopic model
...
As we emphasize above, this also ensures that the \gaia\ measurement $\gparallax_m$ of star $m$ is never used,
even indirectly, in generating the spectrophotometric estimate $\sparallax_m$ for that star $m$.
This, in turn, ensures statistical independence of the
parallax estimates generated here from the \gaia\ parallaxes.
The two could even be combined na\"ively in an inference.
...
\bibitem[Arenou \etal(2018)]{arenou} Arenou, F., Luri, X., Babusiaux, C., \etal\ 2018, \aap, 616, A17 
...
\bibitem[Eilers \etal(2018)]{eilers} Eilers,~A.~C., Hogg, D.~W., Rix, H.-W., \& Ness, M. 2019, \apj, 871, 120
...
\bibitem[Helmi \etal(2018)]{helmi} Helmi, A., Babusiaux, C., Koppelman, H.~H., \etal\ 2018, arXiv:1806.06038 
...
\bibitem[Leung \& Bovy(2019)]{leung} Leung, H.~W., Bovy, J., 2019, arXiv:1902.08634
...
\bibitem[Queiroz \etal(2018)]{que} Queiroz, A.~B.~A., Anders, F., Santiago, B.~X., \etal\ 2018, \mnras, 476, 2556.
\bibitem[Schultheis \etal(2014)]{sch} Schultheis, M., Zasowski, G., Allende Prieto, C., \etal\ 2014, \aj, 148, 24
\bibitem[Skrutskie \etal(2006)]{zmass} Skrutskie, M.~F., Cutri, R.~M., Stiening, R., \etal\ 2006, \aj, 131, 1163 
\bibitem[Trick \etal(2018)]{trick} Trick, W.~H., Coronado, J., \& Rix, H.-W.\ 2018, arXiv:1805.03653 
...
\tablecaption{The generated spectrophotometric parallaxes and their uncertainties..
This \tablename\ is just a snippet; the full catalog of all 44784 objects in the combined Parent Samples
is available online. In order to reproduce the {\figurename s} in this \documentname, this catalog
should be joined to the \gaia, \zmass, \wise\, and \apogee\ Catalogs by \zmass\ ID.\label{tab:output}}
\tablehead{\dcolhead{\texttt{2MASS\_ID}} & \dcolhead{\texttt{Gaia\_parallax}} & \dcolhead{\texttt{Gaia\_parallax\_err}} & \dcolhead{\texttt{spec\_parallax}} & \dcolhead{\texttt{spec\_parallax\_err}} & \dcolhead{\texttt{training\_set}} & \dcolhead{\texttt{sample}}}
\startdata
...
... & ... & ... & ... & ... & ... & ...  \\
 \enddata
\tablecomments{All floating-point numbers are angles in mas.}
\end{deluxetable*}
\end{longrotatetable}
