...
% - related: Switch the model to be a model for parallax directly.
...
\begin{eqnarray}
\varpi_n &=& \exp(\theta\cdot x_n) + \mbox{noise}
\\
\chi^2(\theta) &\equiv& \sum_{n=1}^N \frac{[\varpi_n - \exp(\theta\cdot x_n)]^2}{\sigma_n^2}
\quad ,
\end{eqnarray}
where
$\varpi_n$ is the \gaia\ measurement (adjusted; see below) of the parallax of star $n$,
...
The feature vector $x_n$ contains photometry in a few bands, and all 8000-ish pixels
...
Once the model is optimized, the prediction for a new stellar parallax
$\varpi_m$ for a star $m$ in the validation or test set is
\begin{eqnarray}
\varpi_m &=& \exp(\theta\cdot x_m)
\quad ,
\end{eqnarray}
...
The model trained on the A data is used to predict the $\varpi_m$ values (and hence
the distances and parallaxes) of the B data,
and the model trained on the B data is used to 
predict the $\varpi_m$ values of the A data.
...
(\code{phot_g_mean_mag}) and $({BP}-{RP})_n$ (\code{bp_rp}), which will
...
have the complete set of photometry: $G_n$, $({BP}-{RP})_n$, $J_n$,
...
vector $x_n$ as
\begin{eqnarray}
x_n\T &\equiv& [1, G_n, ({BP}-{RP})_n, J_n, H_n, K_n, W_{1n}, W_{2n}, f_{1n}, f_{2n}, \cdots, f_{Ln}]
\quad ,
\end{eqnarray}
where the 1 permits a linear offset,
the photometry is from \gaia, \zmass, and \wise, respectively,
and
the fluxes in the $L=$ACE \apogee\ spectral pixels for star $n$ are denoted
$f_{1n}$, $f_{2n}$, and so on.
These feature vectors live in a $D$-dimensional space where $D=$HOGG.
For every star $n$ in either of the Training Sets we additionally require---in
addition to these feature vectors---a parallax $\varpi_n$ and uncertainty $\sigma_n$.
