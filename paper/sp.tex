...
% - Make the comparison with Cannon as a function of L1 strength and maybe look at correlation coefficients between parameter vectors.
% - Get model running on ASPCAP outputs and compare in Discussion.
% - Make sure we incorporate / respond to Ness comments from Aug 13 email. And thank Ness.
% - Thank APOGEE telecon and add paragraph about AGB stars and the color-magnitude diagram in the discussion.
% - Describe how we ran The Cannon; discussion of how we compare to The Cannon.
% - Cite and describe Eilers et al.
% - comment on parallax offset being a function of position and color
% - remove Cannon plot for the moment
% - comment on new plot of new precision on parallax compared to Gaia's precision
% - Get figure showing how much better we are than Gaia at large distance and as a function of distance. And write a paragraph about said figure.
...
also must meet $\sigma_{\varpi^{(\rm a)}} < 0.1$, it must be observed by \gaia\ for more than $8$ individual times, i.e. \code{visibility_periods_used}~$\geq~8$, as well as meet the additional quality criterion $\code{astrometric_chi2_al}/\sqrt{\code{astrometric_n_good_obs_al}-5} \leq 35$, which eliminates detections that are not consistent with a single PSF \citep[see][]{calj}, to ensure that the parallax
...
% HOGG: NEW FIGURE -- put some words in the text about the figure. 
\begin{figure}
\centering
