...
This Parent Sample contains ACE HOW MANY STARS? and is shown in \figurename~\ref{fig:parent}.
\begin{figure}
\caption{A visualization of the Parent Sample stars. The left panel
  shows a color--color diagram for all XXXX stars in the combined Parent Sample, colored by
  the \apogee\ pipeline estimates of surface gravity. The right
  panel shows small spectral segments of four example stars,
  chosen to span metallicity and surface gravity.\label{fig:parent}}
\end{figure}
...
\begin{figure}
\caption{A visualization of the Training Set stars. The left panel shows
  same color--color diagram as in \figurename~\ref{fig:parent}, but for
  the Training Set only. Note [HOGG SOME DIFFERENCES]. The right panel
  shows HOGG WHAT?\label{fig:training}}
\end{figure}
...
\begin{figure}
\caption{The coefficients of the spectral part of the linear model.\label{fig:spectral}}
\end{figure}
...
In that context: This model is only linear! We could have done much better with
a more flexible model. But choices, execution.

In that context: These features are bad! We could have used feature engineering
or an autoencoder to do a much better job. Definitely should do this in the future.
...
\caption{A map of the kinematics and element abundances in the Milky Way disk.
  This map shows HOGG SOMETHING.\label{fig:disk}}
\end{figure}
...
\software{} % This command is wack
The code used in this project is available from
\url{https://github.com/aceilers/spectroscopic_parallax}.
It is released under the MIT (HOGG CHECK) open-source software license.
This research additionally utilized:
\code{Astropy} (\citealt{astropy, astropy2}),
\code{IPython} (\citealt{ipython}),
\code{matplotlib} (\citealt{matplotlib}),
\code{numpy} (\citealt{numpy}), and
\code{scipy} (\citealt{scipy}).

\facilities{\sdssiv, \apogee, \apogee-2, \gaia, \zmass, \wise}
